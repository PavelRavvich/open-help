...
public class UserController {
    ...
    public ResponseEntity<ListDto<UserDto>> list(@NotNull @RequestBody @Valid UserFilterDto filter) {
        Pageable pagination = PageRequest.of(filter.getPageNumber(), filter.getPageSize());
        Page<User> page = userService.list(pagination, userMapper.toUserFilter(filter));
        return ResponseEntity.ok(ListDto.<UserDto>builder()
    ...
    public ResponseEntity<UserDto> get(@NotNull @PathVariable(name = "id") Long userId) {
        return ResponseEntity.ok(userMapper.userToUserDto(userService.findById(userId)));
    }

    @PostMapping("/create")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Long> create(@NotNull @RequestBody @Valid SignUpRequestDto req) {
        return ResponseEntity.ok(userService.create(authMapper.signUpRequestDtoToUser(req), req.getIsEnabled()));
    }
    ...
    public ResponseEntity<Long> update(@NotNull @PathVariable(name = "id") Long userId,
        ...
        return ResponseEntity.ok(userService.update(userId, authMapper.signUpRequestDtoToUser(req)));
    }
